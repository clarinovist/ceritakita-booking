                    ...
                    <div key={idx} className="bg-white p-3 rounded border flex flex-col gap-2">
                      ...
                      {(p.proof_url || p.proof_filename || p.proof_base64) && (
                        <div className="flex flex-col gap-2">
                          <div
                            className="relative h-32 w-full max-w-[200px] self-start border rounded bg-white overflow-hidden cursor-pointer hover:opacity-90 transition-opacity"
                            onClick={() => setSelectedImage(p.proof_url || (p.proof_filename ? `/api/uploads/payment-proofs/${p.proof_filename}` : p.proof_base64 ?? ''))}
                          >
                            <Image
