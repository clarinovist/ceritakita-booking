...
export interface WhatsAppCsMetrics {
  drafts: {
    total: number;
    sent: number;
    edited: number;
  ...
  conversion: {
    chatWithBooking: number;
    totalActiveChats: number;
    conversionRatePercent: number;
  };
  aiEvents: Array<{ event_type: string; count: number }>;
}

export function getWhatsAppCsMetrics(): WhatsAppCsMetrics {
  const db = getDb();
  ...
  `).get() as {
    total: number; sent: number; edited: number; rejected: number;
    approved_only: number; pending: number;
  };

  // 2. % drafts sent without edit (via whatsapp_ai_events: approve vs edit)
  const eventStats = db.prepare(`
    SELECT event_type, COUNT(*) as count
    FROM whatsapp_ai_events
    WHERE event_type IN ('approve', 'edit')
      AND created_at >= datetime('now', '-30 days')
    GROUP BY event_type
  `).all() as Array<{ event_type: string; count: number }>;

  const approveCount = eventStats.find(e => e.event_type === 'approve')?.count || 0;
  const editCount = eventStats.find(e => e.event_type === 'edit')?.count || 0;
  const totalSent = approveCount + editCount;
  const sentWithoutEdit = totalSent > 0
    ? Math.round((approveCount / totalSent) * 100)
    : 0;

  // 3. First-response time (inbound → first outbound, last 30 days)
  const responseTimes = db.prepare(`
    SELECT
      m1.conversation_id,
    ...
    LIMIT 500
  `).all() as Array<{
    conversation_id: string; inbound_at: string; first_outbound_at: string;
  }>;
  ...
  if (responseTimes.length > 0) {
    ...
    avgResponseMinutes = Math.round(diffs.reduce((s, d) => s + d, 0) / diffs.length);
    medianResponseMinutes = Math.round(diffs[Math.floor(diffs.length / 2)] || 0);
  }

  // 4. Chat → booking conversion (last 30 days)
  const convStats = db.prepare(`
    SELECT
      (SELECT COUNT(*) FROM whatsapp_conversations
       WHERE booking_id IS NOT NULL AND last_message_at >= datetime('now', '-30 days')
      ) as with_booking,
      (SELECT COUNT(*) FROM whatsapp_conversations
       WHERE last_message_at >= datetime('now', '-30 days') AND status != 'archived'
      ) as total_active
  `).get() as { with_booking: number; total_active: number };

  const conversionRate = convStats.total_active > 0
    ? Math.round((convStats.with_booking / convStats.total_active) * 100)
    : 0;
  ...
  return {
    ...
    conversion: {
      chatWithBooking: convStats.with_booking || 0,
      totalActiveChats: convStats.total_active || 0,
